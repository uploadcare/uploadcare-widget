...
class PreviewTabMultiple extends BasePreviewTab {
  ...
  __fileDone(file, info) {
    ...
    if (info.isImage) {
      ...
      filePreview = $(
        "<svg width='32' height='32'><use xlink:hPreviewTabMultiple='#uploadcare--icon-file'/></svg>"
      )
  ...
  __fileFailed(file, error, info) {
    ...
    filePreview = $(
      "<svg width='32' height='32'><use xlink:hPreviewTabMultiple='#uploadcare--icon-error'/></svg>"
    )
      .attr('role', 'presentation')
      .attr('class', 'uploadcare--icon uploadcare--file__icon')
