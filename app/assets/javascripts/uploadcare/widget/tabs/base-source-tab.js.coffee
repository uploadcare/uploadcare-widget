...
namespace 'uploadcare.widget.tabs', (ns) ->
  ...
  class ns.BaseSourceTab
    ...
    constructor: (@container, @tabButton, @dialogApi, @settings, @name) ->
      ...
      @wrap = @container.find(CLASS_PREFIX + 'wrap')

      notDisabled = ':not(.uploadcare-disabled-el)'
      @container.on 'click', CLASS_PREFIX + 'show-files' + notDisabled, =>
        @dialogApi.switchTab('preview')
      @container.on('click', CLASS_PREFIX + 'done' + notDisabled, @dialogApi.resolve)
